...
// use std::thread;
// use aes_gcm::{Aes256Gcm, AesGcm, Key, KeyInit, Nonce, Aes128Gcm};
// use aes_gcm::aead::consts::U12;
// use aes_gcm::aead::OsRng;
// use aes_gcm::aes::Aes128;
// use log::{debug, info};
// use crate::network::arp_registry::{ArpRegistry, ArpRegistryInit};
...
//         arp_table: &Arc<Mutex<ArpTable<u8, ArpRegistry<LoRaPacket, Aes128Gcm>>>>,
...
//                     if let Some(device) = arp_table.get_registry(packet.header.source_addr) {
...
//                         arp_table.insert_registry(packet.header.source_addr, ArpRegistry::new(Some(packet.header.source_addr), DeviceStatus::Unauthorized));
//                         let device = arp_table.get_registry(packet.header.source_addr);
