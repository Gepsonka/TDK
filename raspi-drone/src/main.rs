...
use crate::network::arp_table::{ArpTable, ArpTableInit};
use crate::network::lora::Bandwidth;
use crate::network::lora::{self, LoRa};
use crate::network::packet::LoRaPacket;
use crate::network::queue::Queue;
use aes_gcm::aead::consts::U12;
use aes_gcm::aead::OsRng;
use aes_gcm::aes::Aes128;
use aes_gcm::{AeadCore, Aes128Gcm, Aes256Gcm, AesGcm, KeyInit};
use core::time;
use env_logger;
...
fn main() {
    ...
    lora_device.reset();
    info!(
        "LoRa version is correct: {:x}",
        lora_device.check_version().unwrap()
    );

    lora_device
        .set_opmod(lora::OperationMode::SX127x_MODE_SLEEP)
        .unwrap();
    lora_device.set_frequency(437200012).unwrap();
    lora_device.reset_fifo().unwrap();
    lora_device
        .set_opmod(lora::OperationMode::SX127x_MODE_STANDBY)
        .unwrap();
    lora_device
        .set_bandwidth(Bandwidth::SX127x_BW_125000)
        .unwrap();
    lora_device
        .set_header_type(lora::HeaderMode::Explicit)
        .unwrap();
    lora_device
        .set_modem_config_2(lora::SpreadFactor::SX127x_SF_9)
        .unwrap();
    lora_device.set_syncword(0x12).unwrap();
    lora_device.set_preamble_length(8).unwrap();
    lora_device
        .set_bandwidth(Bandwidth::SX127x_BW_250000)
        .unwrap();
    lora_device
        .set_opmod(lora::OperationMode::SX127x_MODE_RX_CONT)
        .unwrap();

    info!("LoRa init done!");

    let arp_table_arc: Arc<Mutex<ArpTable<u8, ArpRegistry<LoRaPacket, Aes128Gcm>>>> =
        Arc::new(Mutex::new(network::arp_table::ArpTable::new()));
    let lora_arc = Arc::new(Mutex::new(lora_device));
    let tx_queue_arc = Arc::new(Mutex::new(network::transmit_queue::TransmitQueue::<
        LoRaPacket,
    >::new()));
    // let rx_queue_arc = Arc::new(Mutex::new(network::receive_queue::ReceiveQueue::new()));
    let black_list_arc = Arc::new(Mutex::new(network::blacklist::BlackList::<u8>::new()));

    network::transmit_queue::TransmitQueue::packet_tx_thread(&tx_queue_arc, &lora_arc);
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            packet_to_tx = LoRaPacket::try_from(vec![
